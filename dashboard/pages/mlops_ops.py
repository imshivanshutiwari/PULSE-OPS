...
# ---------------------------------------------------------------------------
MODELS = ['adult/xgboost', 'wine_quality/xgboost', 'bike_sharing/lightgbm', 'german_credit/neural_net']
...
def build_viz01():
    model_info = [
        {
            'name': 'adult/xgboost', 'version': 'v3.2',
            'accuracy': 0.873, 'f1': 0.861,
            'last_train': '2024-05-12', 'status': 'HEALTHY',
            'drift': 0.042, 'requests': 12847, 'color': SUCCESS,
        },
        {
            'name': 'wine_quality/xgboost', 'version': 'v2.1',
            'accuracy': 0.791, 'f1': 0.778,
            'last_train': '2024-05-10', 'status': 'HEALTHY',
            'drift': 0.063, 'requests': 4321, 'color': SUCCESS,
        },
        {
            'name': 'bike_sharing/lightgbm', 'version': 'v4.0',
            'accuracy': 0.912, 'f1': 0.905,
            'last_train': '2024-05-08', 'status': 'DEGRADED',
            'drift': 0.187, 'requests': 28943, 'color': WARNING,
        },
        {
            'name': 'german_credit/neural_net', 'version': 'v1.5',
            'accuracy': 0.816, 'f1': 0.803,
            'last_train': '2024-05-14', 'status': 'HEALTHY',
            'drift': 0.031, 'requests': 7652, 'color': SUCCESS,
        },
    ]
    ...
    for m in model_info:
        ...
        card = html.Div(
            ...
            children=[
                html.Div(m['name'], style={'color': ACCENT_GRN, 'fontSize': '13px', 'fontWeight': 'bold'}),
                html.Div(m['version'], style={'color': TEXT_DIM, 'fontSize': '10px', 'marginBottom': '8px'}),
                html.Div([
                    html.Span('● ', style={'color': status_color}),
                    html.Span(m['status'], style={'color': status_color, 'fontSize': '11px', 'fontWeight': 'bold'}),
                ], style={'marginBottom': '6px'}),
                html.Div([
                    html.Div([html.Span('ACC  ', style={'color': TEXT_DIM}),
                              html.Span(f"{m['accuracy']:.3f}", style={'color': TEXT})]),
                    html.Div([html.Span('F1   ', style={'color': TEXT_DIM}),
                              html.Span(f"{m['f1']:.3f}", style={'color': TEXT})]),
                    html.Div([html.Span('DRIFT ', style={'color': TEXT_DIM}),
                              html.Span(f"{m['drift']:.3f}",
                                        style={'color': WARNING if m['drift'] > 0.1 else SUCCESS})]),
                    html.Div([html.Span('REQS  ', style={'color': TEXT_DIM}),
                              html.Span(f"{m['requests']:,}", style={'color': INFO})]),
                    html.Div([html.Span('TRAIN ', style={'color': TEXT_DIM}),
    ...
    return html.Div([
        html.Div('VIZ01 — Live Model Status Board',
...
def build_viz02():
    ...
    for i, (model, (acc, f1)) in enumerate(perf_data.items()):
        ...
        fig.add_trace(go.Scatter(
            x=dates, y=acc, name=f'{short} ACC',
            line=dict(color=color, width=2),
            mode='lines',
        ))
        fig.add_trace(go.Scatter(
            x=dates, y=f1, name=f'{short} F1',
            line=dict(color=color, width=1.5, dash='dot'),
            mode='lines',
        ))
    layout = {**PLOT_LAYOUT}
    layout['title'] = dict(text='VIZ02 — Model Performance Timeline (30d)', font=dict(color=ACCENT_GRN, size=12))
    ...
    for idx, (label, val, warn_thresh, ok_thresh) in enumerate(gauge_specs, 1):
        ...
        fig.add_trace(go.Indicator(
            ...
            gauge=dict(
                ...
                bordercolor=BORDER,
                steps=[
                    dict(range=[0, ok_thresh if label != 'Quality Score' else warn_thresh],
                         color=ACCENT_DIM),
                    dict(range=[ok_thresh if label != 'Quality Score' else warn_thresh,
...
def build_viz04():
    log_data = [
        ('2024-05-14 09:12', 'drift_threshold', 'german_credit/neural_net', 'v1.4→v1.5', 'PASS', '14m 32s'),
        ('2024-05-12 14:33', 'scheduled_daily',  'adult/xgboost',           'v3.1→v3.2', 'PASS', '8m 17s'),
        ('2024-05-10 07:55', 'drift_threshold',  'wine_quality/xgboost',    'v2.0→v2.1', 'PASS', '6m 04s'),
        ('2024-05-08 18:01', 'perf_degradation', 'bike_sharing/lightgbm',   'v3.9→v4.0', 'PASS', '22m 11s'),
        ('2024-05-07 11:22', 'manual',            'adult/xgboost',           'v3.0→v3.1', 'PASS', '9m 48s'),
        ('2024-05-05 03:40', 'scheduled_daily',  'german_credit/neural_net','v1.3→v1.4', 'PASS', '13m 59s'),
        ('2024-05-03 16:15', 'drift_threshold',  'bike_sharing/lightgbm',   'v3.8→v3.9', 'FAIL', '4m 02s'),
        ('2024-05-01 09:00', 'scheduled_daily',  'wine_quality/xgboost',    'v1.9→v2.0', 'PASS', '7m 33s'),
    ]
    ...
    layout = {**PLOT_LAYOUT}
    layout['title'] = dict(text='VIZ04 — Retraining Activity Log', font=dict(color=ACCENT_GRN, size=12))
    layout['height'] = 310
    layout.pop('xaxis', None)
    layout.pop('yaxis', None)
    fig.update_layout(**layout)
...
def build_viz05():
    ...
    flows = [
        ...
        ('feature_engineering',   now - datetime.timedelta(hours=1, minutes=50), 67,  SUCCESS,  'Completed'),
        ('model_training',        now - datetime.timedelta(hours=1, minutes=10), 1398, ACCENT_GRN,'Completed'),
    ...
    for i, (name, start, dur_sec, color, state) in enumerate(flows):
        ...
        fig.add_annotation(
            x=((start - (now - datetime.timedelta(hours=3))).total_seconds() / 60 +
    ...
    layout = {**PLOT_LAYOUT}
    layout['title'] = dict(text='VIZ05 — Prefect Flow Run Status', font=dict(color=ACCENT_GRN, size=12))
...
def build_viz06():
    ...
    layout = {**PLOT_LAYOUT}
    layout['title'] = dict(text='VIZ06 — MLflow Experiment Tracker (Parallel Coords)', font=dict(color=ACCENT_GRN, size=12))
    layout['height'] = 340
    layout.pop('xaxis', None)
    layout.pop('yaxis', None)
    fig.update_layout(**layout)
...
def build_viz07():
    ...
    fig.add_trace(go.Scatter(x=hours, y=req_rate, fill='tozeroy',
                             fillcolor=ACCENT_DIM, line=dict(color=ACCENT_GRN, width=1.5),
                             name='Req/s'), row=1, col=1)
    fig.add_trace(go.Scatter(x=hours, y=freshness, fill='tozeroy',
                             fillcolor='#0a1e04', line=dict(color=SUCCESS, width=1.5),
                             name='Freshness'), row=2, col=1)
    fig.add_trace(go.Scatter(x=hours, y=latency_p50, line=dict(color=INFO, width=1.5),
                             name='P50 ms'), row=3, col=1)
    fig.add_trace(go.Scatter(x=hours, y=latency_p99, line=dict(color=WARNING, width=1.5),
                             name='P99 ms'), row=3, col=1)

    layout = {**PLOT_LAYOUT}
    layout['title'] = dict(text='VIZ07 — Feature Store Usage Metrics (48h)', font=dict(color=ACCENT_GRN, size=12))
    layout['height'] = 420
    for k in ['xaxis', 'yaxis']:
        layout.pop(k, None)
    for i in range(1, 4):
        suffix = '' if i == 1 else str(i)
        layout[f'xaxis{suffix}'] = dict(gridcolor=BORDER, linecolor=BORDER, tickcolor=BORDER,
                                         zerolinecolor=BORDER, tickfont=dict(color=TEXT_DIM))
        layout[f'yaxis{suffix}'] = dict(gridcolor=BORDER, linecolor=BORDER, tickcolor=BORDER,
                                         zerolinecolor=BORDER, tickfont=dict(color=TEXT))
    fig.update_layout(**layout)
...
layout = html.Div(
    ...
    children=[
        html.Div('◈ MLOps Ops Center', style={
