...
def build_viz14():
    elements = [
        ...
        {'data': {'id': 'a-v1', 'label': 'adult\nv1.0', 'stage': 'archived'}, 'position': {'x': 80, 'y': 60}},
        {'data': {'id': 'a-v2', 'label': 'adult\nv2.0', 'stage': 'archived'}, 'position': {'x': 80, 'y': 160}},
        {'data': {'id': 'a-v3', 'label': 'adult\nv3.2', 'stage': 'production'}, 'position': {'x': 80, 'y': 260}},
        {'data': {'id': 'a-v4', 'label': 'adult\nv3.3', 'stage': 'staging'}, 'position': {'x': 80, 'y': 360}},

        # wine_quality/xgboost versions
        {'data': {'id': 'w-v1', 'label': 'wine\nv1.0', 'stage': 'archived'}, 'position': {'x': 260, 'y': 60}},
        {'data': {'id': 'w-v2', 'label': 'wine\nv2.1', 'stage': 'production'}, 'position': {'x': 260, 'y': 160}},
        {'data': {'id': 'w-v3', 'label': 'wine\nv2.2', 'stage': 'staging'}, 'position': {'x': 260, 'y': 260}},

        # bike_sharing/lightgbm versions
        {'data': {'id': 'b-v1', 'label': 'bike\nv3.8', 'stage': 'archived'}, 'position': {'x': 440, 'y': 60}},
        {'data': {'id': 'b-v2', 'label': 'bike\nv3.9', 'stage': 'archived'}, 'position': {'x': 440, 'y': 160}},
        {'data': {'id': 'b-v3', 'label': 'bike\nv4.0', 'stage': 'production'}, 'position': {'x': 440, 'y': 260}},
        {'data': {'id': 'b-v4', 'label': 'bike\nv4.1', 'stage': 'staging'}, 'position': {'x': 440, 'y': 360}},

        # german_credit/neural_net versions
        {'data': {'id': 'g-v1', 'label': 'german\nv1.3', 'stage': 'archived'}, 'position': {'x': 620, 'y': 60}},
        {'data': {'id': 'g-v2', 'label': 'german\nv1.4', 'stage': 'archived'}, 'position': {'x': 620, 'y': 160}},
        {'data': {'id': 'g-v3', 'label': 'german\nv1.5', 'stage': 'production'}, 'position': {'x': 620, 'y': 260}},
    ...
    return html.Div([
        html.Div('VIZ14 — Model Version Lifecycle Graph',
...
def build_viz15():
    ...
    layout = {**PLOT_LAYOUT}
    layout['title'] = dict(text='VIZ15 — Champion vs Challenger Metrics', font=dict(color=ACCENT_GRN, size=12))
...
def build_viz16():
    ...
    colors = [
        ACCENT_GRN if imp > 0.2 else (WARNING if imp > 0.05 else TEXT_DIM)
    ...
    layout = {**PLOT_LAYOUT}
    layout['title'] = dict(text='VIZ16 — Hyperparameter Importance (Optuna FAnova)', font=dict(color=ACCENT_GRN, size=12))
    layout['height'] = 320
    layout['xaxis'] = {**PLOT_LAYOUT.get('xaxis', {}), 'title': 'Importance Score', 'range': [0, 0.42]}
...
def build_viz17():
    ...
    for model, y_pos in model_y.items():
        ...
        for ev in model_events:
            ...
            fig.add_trace(go.Scatter(
                ...
                y=[y_pos],
                mode='markers+text',
                marker=dict(color=ev[4], size=12, symbol='circle',
    ...
    layout = {**PLOT_LAYOUT}
    layout['title'] = dict(text='VIZ17 — Model Lineage Timeline', font=dict(color=ACCENT_GRN, size=12))
    layout['height'] = 340
    layout['yaxis'] = dict(
