...
def build_viz21():
    ...
    for (row, col), (label, value, unit, vmin, vmax, warn, ok) in zip(positions, gauges):
        ...
        fig.add_trace(
            go.Indicator(
                ...
                number=dict(
                    font=dict(color=color, size=20, family=FONT),
                    suffix=unit if unit in ("%", "ms") else "",
                ),
                gauge=dict(
                    axis=dict(
                        range=[vmin, vmax],
                    ...
                    steps=[
                        ...
                        dict(
                            range=[
                                ok if is_lower_better else warn,
                                warn if is_lower_better else ok,
                            ],
                            color="#1a2010",
                        ),
                    ],
    ...
    return dcc.Graph(
        id="viz21-gauges", figure=fig, config={"displayModeBar": False}, style=CARD_STYLE
    )
...
def build_viz22():
            ...
            children=[
                html.Div(
                    [
                        ...
                        html.Span(
                            name, style={"color": TEXT, "fontSize": "11px", "fontWeight": "bold"}
                        ),
                    ]
                ),
    ...
    summary = html.Div(
        [
            html.Span(
                "System Status: ", style={"color": TEXT_DIM, "fontFamily": FONT, "fontSize": "12px"}
            ),
            html.Span(
                f"{n_healthy}/{n_total} services online ({pct:.0f}%)",
                style={
                    "color": summary_color,
                    "fontFamily": FONT,
                    "fontSize": "12px",
