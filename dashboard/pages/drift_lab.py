...
def build_viz08():
    ...
    if report_path:
        try:
            ...
            return html.Div([
                html.Div('VIZ08 — Evidently Drift Report',
                ...
                html.Iframe(
                    ...
                    style={
                        ...
                        'borderRadius': '4px', 'backgroundColor': BG_CARD,
                    },
                ),
            ], style=CARD_STYLE)
        except Exception:
    ...
    for feat, score, method, result in drift_rows:
        ...
        rows_html.append(html.Tr([
            html.Td(feat,   style={'padding': '4px 10px', 'color': TEXT, 'fontFamily': FONT, 'fontSize': '11px'}),
            html.Td(f'{score:.2f}', style={'padding': '4px 10px', 'color': INFO, 'fontFamily': FONT, 'fontSize': '11px'}),
            html.Td(method, style={'padding': '4px 10px', 'color': TEXT_DIM, 'fontFamily': FONT, 'fontSize': '11px'}),
            html.Td(result, style={'padding': '4px 10px', 'color': color, 'fontFamily': FONT, 'fontSize': '11px', 'fontWeight': 'bold'}),
        ]))
    return html.Div([
        html.Div('VIZ08 — Evidently Drift Report (run `make fetch` to load real report)',
                 style={'color': ACCENT_GRN, 'fontSize': '12px', 'marginBottom': '8px', 'fontFamily': FONT}),
        html.Div([
            html.Div([
                html.Span('Dataset Drift: ', style={'color': TEXT_DIM, 'fontFamily': FONT, 'fontSize': '11px'}),
                html.Span('DETECTED', style={'color': WARNING, 'fontFamily': FONT, 'fontSize': '11px', 'fontWeight': 'bold'}),
                html.Span('   |   Drifted features: ', style={'color': TEXT_DIM, 'fontFamily': FONT, 'fontSize': '11px'}),
                html.Span('3 / 6', style={'color': WARNING, 'fontFamily': FONT, 'fontSize': '11px'}),
                html.Span('   |   Share: ', style={'color': TEXT_DIM, 'fontFamily': FONT, 'fontSize': '11px'}),
                html.Span('50%', style={'color': WARNING, 'fontFamily': FONT, 'fontSize': '11px'}),
            ], style={'marginBottom': '10px', 'padding': '8px', 'backgroundColor': BG_PANEL,
                      'border': f'1px solid {BORDER}', 'borderRadius': '3px'}),
            html.Table([
                html.Thead(html.Tr([
                    html.Th(h, style={'padding': '4px 10px', 'color': ACCENT_GRN,
...
def build_viz09():
    ...
    for feat in FEATURES:
        ...
        fig.add_trace(go.Violin(
            y=ref_norm, x=[feat] * len(ref_norm),
            name='Reference', legendgroup='Reference',
            showlegend=(feat == FEATURES[0]),
            side='negative', line_color=INFO,
            fillcolor='rgba(55,138,221,0.25)', opacity=0.9,
        ))
        fig.add_trace(go.Violin(
            y=cur_norm, x=[feat] * len(cur_norm),
            name='Current', legendgroup='Current',
            showlegend=(feat == FEATURES[0]),
            side='positive', line_color=WARNING,
            fillcolor='rgba(186,117,23,0.25)', opacity=0.9,
        ))
    layout = {**PLOT_LAYOUT}
    layout['title'] = dict(text='VIZ09 — Feature Distribution Comparison (normalised)', font=dict(color=ACCENT_GRN, size=12))
    layout['violingap'] = 0
    layout['violinmode'] = 'overlay'
    layout['height'] = 340
    layout['yaxis'] = {**PLOT_LAYOUT.get('yaxis', {}), 'title': 'Normalised Value', 'zeroline': False}
...
def build_viz10():
    ...
    acc = np.concatenate([
        ...
        np.clip(0.873 - np.linspace(0, 0.06, 30 - drift_day) + rng.normal(0, 0.007, 30 - drift_day), 0.80, 0.92),
    ])
    ...
    fig.add_trace(go.Scatter(x=days, y=acc, name='Accuracy', line=dict(color=ACCENT_GRN, width=2)), row=1, col=1)
    fig.add_trace(go.Scatter(x=days, y=f1, name='F1 Score', line=dict(color=INFO, width=2)), row=1, col=1)
    fig.add_trace(go.Scatter(x=days, y=recall, name='Recall', line=dict(color=WARNING, width=2)), row=1, col=1)
    fig.add_vline(x=days[drift_day].timestamp() * 1000, line_dash='dot', line_color=CRITICAL,
                  annotation_text='Drift detected', annotation_font=dict(color=CRITICAL, size=9))
    ...
    layout = {**PLOT_LAYOUT}
    layout['title'] = dict(text='VIZ10 — Model Performance Degradation', font=dict(color=ACCENT_GRN, size=12))
    layout['height'] = 320
    ...
    for i in range(1, 3):
        s = '' if i == 1 else str(i)
        fig.update_layout(**{
            f'xaxis{s}': dict(gridcolor=BORDER, linecolor=BORDER, tickfont=dict(color=TEXT_DIM, size=9)),
            f'yaxis{s}': dict(gridcolor=BORDER, linecolor=BORDER, tickfont=dict(color=TEXT_DIM, size=9)),
        })
    return dcc.Graph(figure=fig, config={'displayModeBar': False}, style=CARD_STYLE)
...
def build_viz11():
    ...
    fig.add_trace(go.Table(
        ...
        cells=dict(
            ...
            font=dict(color=[TEXT_DIM, [CRITICAL if 'True' in str(v) else TEXT for v in values]],
                      size=10, family=FONT),
            line_color=BORDER,
        ),
    ), row=1, col=2)
    ...
    layout = {**PLOT_LAYOUT}
    layout['title'] = dict(text='VIZ11 — Target Drift Analysis', font=dict(color=ACCENT_GRN, size=12))
    layout['height'] = 310
    ...
    fig.update_layout(
        ...
        yaxis=dict(gridcolor=BORDER, linecolor=BORDER, tickfont=dict(color=TEXT), title='Count (%)'),
    )
    return dcc.Graph(figure=fig, config={'displayModeBar': False}, style=CARD_STYLE)
...
def build_viz12():
    ...
    features_ext = ['age', 'income', 'credit_score', 'debt_ratio',
                    'num_accounts', 'years_employed', 'zip_code', 'loan_amount']
    ...
    layout = {**PLOT_LAYOUT}
    layout['title'] = dict(text='VIZ12 — Data Quality Scorecard', font=dict(color=ACCENT_GRN, size=12))
    layout['height'] = 340
    for k in ['xaxis', 'yaxis']:
        layout.pop(k, None)
    fig.update_layout(**layout)
...
def build_viz13():
    ...
    layout = {**PLOT_LAYOUT}
    layout['title'] = dict(text='VIZ13 — Drift History Calendar (365d)', font=dict(color=ACCENT_GRN, size=12))
    layout['height'] = 220
    layout['xaxis'] = dict(
        title='Week', gridcolor=BORDER, linecolor=BORDER,
        tickfont=dict(color=TEXT_DIM, size=8), tickmode='auto', nticks=12,
    )
    layout['yaxis'] = dict(
...
layout = html.Div(
    ...
    children=[
        html.Div('◈ Drift Analysis Lab', style={
