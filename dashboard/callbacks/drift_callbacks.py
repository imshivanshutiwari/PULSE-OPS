...
from theme import (
    BG_CARD, BORDER, ACCENT_GRN, INFO, WARNING, CRITICAL, SUCCESS, TEXT, TEXT_DIM, FONT,
    PLOT_LAYOUT, CARD_STYLE
)
...
def register_drift_callbacks(app):
    ...
    def update_drift_scores(n_intervals, selected_model):
        ...
        for metric, score in scores.items():
            ...
            items.append(html.Div([
                html.Span(f'{metric}: ', style={'color': TEXT_DIM, 'fontFamily': FONT, 'fontSize': '11px'}),
                html.Span(f'{score:.4f}', style={'color': color, 'fontFamily': FONT,
    ...
    def update_feature_distribution(n_intervals, selected_feature, selected_model):
        rng = np.random.default_rng(
            hash(str(selected_feature) + str(selected_model)) % 10000
            + ((n_intervals or 0) % 100)
        )
        feature_params = {
            'age':            (42, 12, 38, 13),
        ...
        fig.add_trace(go.Histogram(
            x=ref_vals, name='Reference', opacity=0.65,
            marker_color=INFO, nbinsx=35,
            histnorm='probability density',
        ))
        fig.add_trace(go.Histogram(
            x=cur_vals, name='Current', opacity=0.65,
            marker_color=WARNING, nbinsx=35,
            histnorm='probability density',
        ))
        layout = {**PLOT_LAYOUT}
        layout['title'] = dict(
    ...
    def update_drift_alert(n_intervals):
        ...
        if drift_score > 0.20:
            ...
            style = {
                'backgroundColor': '#2a1000', 'border': f'1px solid {WARNING}',
                'color': WARNING, 'fontFamily': FONT, 'fontSize': '11px',
                'padding': '8px 12px', 'borderRadius': '3px', 'marginBottom': '8px',
            }
        elif drift_score > 0.15:
            msg = f'● DRIFT WARNING  bike_sharing/lightgbm  score={drift_score:.3f}  → monitoring closely'
            style = {
                'backgroundColor': '#1a1000', 'border': f'1px solid {WARNING}',
                'color': WARNING, 'fontFamily': FONT, 'fontSize': '11px',
                'padding': '8px 12px', 'borderRadius': '3px', 'marginBottom': '8px',
            }
        else:
            ...
            style = {
                ...
                'color': SUCCESS, 'fontFamily': FONT, 'fontSize': '11px',
                'padding': '8px 12px', 'borderRadius': '3px', 'marginBottom': '8px',
            }
        return msg, style
